...
def run_scf_h2_clean(guess_type='null'):
    R_bohr = 1.4
    S12 = 0.6593
    S_matrix = np.array([[1.0, S12], [S12, 1.0]])

    t11, t12 = 0.7600, 0.2365
    h11 = t11 + (-1.2266) + (-0.6538)
    h12 = t12 + (-0.5974) + (-0.5974)
    H_core_matrix = np.array([[h11, h12], [h12, h11]])

    int_2e = {
        (1,1,1,1): 0.7746, (2,2,2,2): 0.7746,
        (1,1,2,2): 0.5697, (2,2,1,1): 0.5697,
        (2,1,1,1): 0.4441, (1,2,1,1): 0.4441,
        (2,1,2,2): 0.4441, (1,2,2,2): 0.4441,
        (2,1,2,1): 0.2970, (1,2,1,2): 0.2970,
    }
    ...
    elif guess_type == 'h2plus':
        norm = 1.0 / np.sqrt(2 * (1 + S12))
        C_guess = np.array([[norm], [norm]])
        P_matrix = 2.0 * C_guess @ C_guess.T

    iteration = 0
    is_converged = False
    convergence_threshold = 1e-6

    print(f"--- SCF Start (Guess: {guess_type}) ---")
...
run_scf_h2_clean('null')
run_scf_h2_clean('h2plus')