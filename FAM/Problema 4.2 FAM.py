...
psi4.set_output_file('problem2_products.dat')
methods_list = ['hf', 'mp2', 'b3lyp']
basis_set = '6-31G'

print(f"--- Problem 2: Products Optimization ({basis_set}) ---")

for method in methods_list:
    print(f"\nRunning Optimization: {method.upper()}/{basis_set} ...")
    products = psi4.geometry("""
        -1 1
        C
        H 1 1.07
        H 1 1.07 2 109.5
        H 1 1.07 3 109.5 2 120.0
        Br 1 2.75 3 70.0  2 60.0
        Cl 1 2.10 3 109.5 2 -120.0
    """)
    final_energy = psi4.optimize(f'{method}/{basis_set}', molecule=products)
    print(f"  -> Final Energy: {final_energy:.6f} a.u.")
    geom_bohr = products.geometry().to_array()
    ...
    for i in range(products.natom()):
        symbol = products.symbol(i)
        x = geom_ang[i][0]
        y = geom_ang[i][1]
        z = geom_ang[i][2]
...
print("\nOptimizations complete. Data in 'problem2_products.dat'.")